function [hatrho_J,hateta5_J,hateta5_full,var_source_Gamma1,var_source_Gamma1_2_2,var_source_Gamma3,jn,var_source_Gamma1_2,var_source_Gamma1_full] = eta5_estimator(e,omega,alpha,r,Jn)
    % initial
    n=size(e,1);
    jn=size(Jn,1);
    ...
    for idx_i = 1:n
        ...
        g51_full(idx_i)=calculate_g51_J(e,idx_i,n,nchoosek(1:n,3));
        g11_full(idx_i)=calculate_g11_J(e,idx_i,n,nchoosek(1:n,2));
    end
    var_source_Gamma1 =  max(0,hatrho_J^(-4)).*mean((3.*(g51-hatU5star_J)-4.*hatU1star_J.*(g11-hatU1star_J)).^2)/n;
    var_source_Gamma1_2 =  max(0,hatrho_J^(-4)).*(var_V_J_star);
    var_source_Gamma1_full = max(0,hatrho_J^(-4)).*mean((3.*(g51_full-mean(g51_full))-4.*mean(g11_full).*(g11_full-mean(g11_full))).^2)/n;
