    ...
    for city_idx = 1:length(city_sizes)
        ...
        for rho_idx = 1:length(rhosizes)
            ...
            % Generate error terms for two city sizes
                [e_ij_original,e_ij,omega_ij] = WE_null(city_size, rhosize,"normal",5);
            %E2 = WE_null(city_size + 100, rhosize + 0.1);
                ...
                %[hateta3_wq_J,var_wq_J]=incomplete_Us_NO_debias_standz(city_size,Jn,e_ij,5);
                
                bound_1 = sqrt(log(city_size)/city_size);
                bound_2 = (log(city_size)/city_size);
                fprintf('sample size %s alpha %d iteration %f.\n', city_size, alpha,i)
                results(i, :) = [hatrho_J,hateta3_J,hateta3_full,var_source_Gamma1,var_source_Gamma2,var_source_Gamma3,city_size,rhosize,alpha,jn,var_source_Gamma1_2,var_source_Gamma1_full];
            end
...
T = array2table(test_stat_1, 'VariableNames', {'hatrho_J','hateta3_J','hateta3_full','var_source_Gamma1', 'var_source_Gamma2', 'var_source_Gamma3', 'n','rho','alpha','jn','var_source_Gamma1_2','var_source_Gamma1_full'});
writetable(T, "eta5_param_r3_normal_nondeg_alter_0.5_sparsity.csv", 'Delimiter', ';','WriteVariableNames', true);
