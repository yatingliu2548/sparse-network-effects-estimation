...
%rho1sizes = [0.5,1];%,0.6,0.8,0.9
rhosizes=[0.05,0.1,0.4,0.8,1];
...
mean=0.5;
name=sprintf('eta5/eta5_normal_two_sample_%d_%d.csv', jobid, taskid);
disp(name)
...
for alpha_idx=1:length(alphasizes)
    ...
    [e_ij_original2,e_ij2,omega_ij2] = WE_null(citysize2, rhosize2,"normal",5,sigma);
    ...
    for city_idx = 1:length(city_sizes)
        ...
        for rho_idx = 1:length(rhosizes)
            ...
            % Generate error terms for two city sizes
                [e_ij_original,e_ij,omega_ij] = WE_null(city_size, rhosize,"normal",5,sigma);
